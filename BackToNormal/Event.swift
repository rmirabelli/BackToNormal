...
extension Event {
    ...
    static func createInitialValues() {
        ...
        let poop = UserDefaults(suiteName: "group.rmirabelli.backtonormal")
        poop?.set(try? PropertyListEncoder().encode(initialEvents), forKey: "events")
    }

    static func getValues() -> [Event] {
        let poop = UserDefaults(suiteName: "group.rmirabelli.backtonormal")
        if let data = poop?.value(forKey:"events") as? Data {
